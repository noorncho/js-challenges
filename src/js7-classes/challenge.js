...
export class Modal {
  ...
  displayModal(){
    // if(this.htmlRef.classList.contains("show")){
    //   this.htmlRef.classList.remove("show");
    //   this.htmlRef.classList.add("hide");
    // }else{
    //   this.htmlRef.classList.remove("hide");
    //   this.htmlRef.classList.add("show");
    // }
...
export class BookShelf {
  ...
  get latestBook(){
    const length = this._booksOnShelf.length - 1;
    return this._booksOnShelf[length];
  }
...
export class BankAccount {
  ...
  deposit(toDeposit){
    const convertToNum = Number(toDeposit);

    if(!convertToNum|| convertToNum < 0){
      return "Invalid input, unable to deposit";
    }
    this._balance += convertToNum;
    return this._balance;
  }
  ...
  withdraw(toWithdraw){
    const convertToNum = Number(toWithdraw);
    
    if(!convertToNum || convertToNum < 0){
      return "Invalid input, unable to withdraw";
    }

    if(convertToNum > this._balance){
